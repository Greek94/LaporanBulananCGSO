    ...
    if (sh) {
      ...
      if (values.length >= 2) {
        ...
        const idx = function(name) {
          return headers.findIndex(function(h) { return h.toLowerCase() === name.toLowerCase(); });
        };
          ...
          for (let r = 1; r < values.length; r++) {
            ...
            if (Object.prototype.toString.call(raw) === '[object Date]' && !isNaN(raw.getTime())) {
              ...
              if (!pid) {
                ...
                if (x) pid = x[3] + '-' + String(x[2]).padStart(2, '0');
              }
              if (!pid) pid = text;
            }

            if (pid) addPeriod_(pid, ist >= 0 ? values[r][ist] : '');
          }
        }
      }
    }
